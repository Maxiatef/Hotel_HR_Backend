...
export class UserService {
  ...
  async create(hotelId: string, dto: CreateUserDto) {
    const passwordHash = await bcrypt.hash(dto.password, 10);
    const { password, ...rest } = dto;
    const entity = this.repo.create({ ...rest, hotelId, passwordHash });
  ...
  async update(id: string, hotelId: string, dto: UpdateUserDto) {
    ...
    if (dto.password) {
      (record as any).passwordHash = await bcrypt.hash(dto.password, 10);
